</head>

<body>
    <div class="container-fluid">

        <header>
            <h1 class="text-center"><a styles="none" href="www/index.php">SOPEYO</a></h1>
            <ul class="nav justify-content-around bg-primary">
                <li class="nav-item">
                    <a class="nav-link text-light" aria-current="page" href="www/views/customer.php">Devis</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link  text-light" href="#">Comptabilité</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link  text-light" href="#">Résultats</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-light" href="#">Chantiers</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle text-light" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        Ressources
                    </a>
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="www/views/products.php">Produits</a></li>
                        <li><a class="dropdown-item" href="#">Terrasses</a></li>
                        <li>
                            <hr class="dropdown-divider">
                        </li>
                        <li><a class="dropdown-item" href="#">DTU</a></li>
                    </ul>
                </li>
            </ul>
        </header>
        <div style="min-height: 50em">